		...
		int desired_state, void * _chimpl, void * _socket_state)
{
	...
	fprintf(stderr, "on_socket_update: sock=%d easy=%p desired_state=%s\n",
		sock, easy, get_socket_state_descrip(desired_state));
	...
	if (desired_state == CURL_POLL_REMOVE) {
		...
		fprintf(stderr, "Changing state from %s to %s\n",
				get_socket_state_descrip(*socket_state),
				get_socket_state_descrip(desired_state));
...
{
	size_t bytes = size_per_record * num_records;
	fprintf(stderr, "entering %s() with %d bytes\n", __func__, (int)bytes);
	fprintf(stderr, "---------------\n");
	char buf[2048];
	size_t printed_byte_count = 0;
	size_t remaining = bytes - printed_byte_count;
	while (remaining) {
		size_t n = std::min(remaining, sizeof(buf) - 1);
		memcpy(buf, ((char *)data) + printed_byte_count, n);
		buf[n] = 0;
	...
	fprintf(stderr, "exiting %s(), having handled %d bytes\n", __func__, (int)bytes_handled);

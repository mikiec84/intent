#ifndef _FOUNDATION_EVENT_H_
#define _FOUNDATION_EVENT_H_

#include <string>
#include <vector>

#include "foundation/lifecycle_semantics.h"
#include "foundation/thread_semantics.h"

namespace intent { namespace foundation { namespace events {
 ...
 */
char const * get_symbolic_name(int ecode);

/**
 * @return the {@link Severity} for a particular event.
 */
severity_t get_severity(int ecode);
 ...
 */
char const * get_msg(ecode_t event);
 ...
 */
size_t get_std_id_repr(int ecode, char * buf, size_t buflen);
static const size_t MIN_STD_ID_REPR = 11;

/**
 * How many args should be used with this event?
 */
int get_arg_count(int ecode);

/**
 * Call at startup to make events in another component known to the system,
 * like this:
 *
 *     publish_event_data(dat, dat_count);
 */
NOT_THREADSAFE CALLABLE_IN_LIFECYCLE_PHASES(before main)
...
items_t const & get_event_data_items();
event_data const * find_event(int code);

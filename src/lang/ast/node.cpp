...
void node::add_child(node * child) {
    ...
    if (child_count == allocated_child_count) {
        ...
        auto start_unallocated = &children[0] + (child_count * unit_size);
        auto end_unallocated = &children[0] + (new_count * unit_size);
        memset(start_unallocated, 0, end_unallocated - start_unallocated);

...
size_t libcurl_callbacks::on_receive_data(void * data, size_t size_per_record, size_t num_records, void * _rimpl)
{
    ...
    if (_rimpl) {
        ...
        auto func = rimpl.receive_func;
        if (func) {
            //bytes_handled = func(*rimpl.response, data, size_per_record * num_records);
        }
    }
        ...
        uint64_t sent_so_far)
{
    ...
    if (_rimpl) {
        auto & rimpl = *reinterpret_cast<response::impl_t *>(_rimpl);
        auto func = rimpl.progress_func;
        if (func) {
            //err = func(*rimpl.response, expected_receive_total, received_so_far,

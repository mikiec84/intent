...
TEST(cmdline_test, invalid_ctor_with_no_args) {
	...
	EXPECT_STREQ("intent", x.get_program_name());
}

TEST(cmdline_test, help_needed_with_help_explicitly_requested) {
	char const * args[] = {"intent", "--help"};
	cmdline x(2, args);
	ASSERT_TRUE(x.help_needed());
}

TEST(cmdline_test, help_needed_with_error) {
	char const * args[] = {"intent", "--port1"};
	cmdline x(2, args);
	ASSERT_TRUE(x.help_needed());
	// Should get error about needing a value for option
	expect_errors_contain(x.get_errors(), "Expected --port1 option to be followed by");
}

TEST(cmdline_test, bogus_flag) {
	char const * args[] = {"intent", "--bogus"};
	cmdline x(2, args);
	expect_errors_contain(x.get_errors(), "--bogus");
}

TEST(cmdline_test, non_numeric_port_error) {
	char const * args[] = {"intent", "--port1", "abc"};
	cmdline x(3, args);
...
TEST(cmdline_test, out_of_range_port_error) {
	char const * args[] = {"intent", "--port1", "1234567"};
	cmdline x(3, args);

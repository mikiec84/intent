#ifndef _FOUNDATION_DBC_H_
#define _FOUNDATION_DBC_H_
...
#define PRECONDITION(expr) if (!(expr)) \
	CONTRACT_VIOLATION_ACTION(\
			intent::foundation::event_codes::E_PRECONDITION_EXPR1_VIOLATED, #expr)
...
#define CHECK(expr) if (!(expr)) \
	CONTRACT_VIOLATION_ACTION(\
			intent::foundation::event_codes::E_CHECK_EXPR1_VIOLATED, #expr)
...
#define POSTCONDITION(expr) \
	BOOST_SCOPE_EXIT_ALL(&) { \
		if (!(expr)) { \
			CONTRACT_VIOLATION_ACTION( \
					intent::foundation::event_codes::E_POSTCONDITION_EXPR1_VIOLATED, #expr); \
		} \
	}

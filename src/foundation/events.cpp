...
items_t & _get_event_data_items() {
	static items_t items;
	return items;
}

items_t const & get_event_data_items() {
	return _get_event_data_items();
}
...
bool publish_event_data(event_data const * data, size_t count) {
	...
	items_t & items = _get_event_data_items();
	if (items.empty()) {
		items.reserve(256);
	} else {
...
event_data const * find_event(int ecode) {
	items_t const & items = get_event_data_items();
...
char const * get_msg(ecode_t ecode) {
	auto e = find_event(ecode);
	return e ? e->msg : "";
}
...
char const * get_severity_label(int ecode) {
	if (get_component(ecode) == kc_posix) {

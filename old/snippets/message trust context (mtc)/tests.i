...
zero: +fixture
    Return mtc.Make(denied = -1)
    
partial: +fixture
    := mtc.Make(integrity | confidentiality)

Given an mtc with all flags undefined (undefined):
    abbrevs should equal "?":
        assert abbrevs == '?'
    should return "?" when converted to a str:
        assert str(undefined) == "?"
    it should return assert '?' == str(undefined)

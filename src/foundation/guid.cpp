...
inline uint16_t rand16(std::mt19937 & randomizer) {
	return static_cast<uint16_t>(randomizer());
}
...
void generate_guid(char * buf, size_t buflen) {
	...
	static unsigned seed = std::chrono::system_clock::
			now().time_since_epoch().count();
	static std::mt19937 rnd(seed);
	sprintf(buf, "%04x%04x-%04x-%04x-%04x-%04x%04x%04x",
		    rand16(rnd), rand16(rnd),
		    rand16(rnd),
		    ((rand16(rnd) & 0x0fff) | 0x4000), // hex 4xxx (4 = UUID version)
		    rand16(rnd) % 0x3fff + 0x8000,     // hex in range [0x8000, 0xbfff]
		    rand16(rnd), rand16(rnd), rand16(rnd));
}

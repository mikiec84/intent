...
struct file_lines::data_t {
	...
	data_t(char const * _fname, bool ltrim, bool rtrim, size_t gulp_size) :
		path(_fname), f(NULL),	flen(0), foffset(0),
		gulp_size(gulp_size ?
		...
		if (f) {
			...
			if (flen) {
				...
				cleanup();
				throw ERROR_EVENT(E_INPUT_FILE1_EMPTY, _fname);
			}
		} else {
			throw ERROR_EVENT(E_INPUT_FILE1_UNREADABLE, _fname);
		}
	}
	...
	bool gulp() {
		...
		if (gulp_count == 1) {
			if (data_seems_binary(buf, buf + min(bytes_read,
				...
				cleanup();
				throw ERROR_EVENT(E_FILE1_BAD_SEEMS_BINARY, path);
			}
		}
		...
		if (end == buf) { // file's not exhausted, full buffer had 0 LF
			cleanup();
			throw ERROR_EVENT(E_FILE1_BAD_HUGE_LINE_2BYTES, path, bytes_read);
		}
...
char const * file_lines::get_path() const {
